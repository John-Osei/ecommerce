...
from core.forms import NewsletterForm, ProductReviewForm
from django.contrib import messages
from django.db.models import Avg
from taggit.models import Tag
from django.template.loader import render_to_string
from userauths.models import ContactUs, Profile, SubscribedUsers
...
from core.models import Product,ProductImages,Category,Vendor,CartOrder,CartOrderItem,Address,ProductReview, Wishlist
...
def paginate(request):
    ...
    result= Product.objects.filter(title= title)[starting_number:ending_number]
...
def category_list_view(request, tag_slug=None):
    categories = Category.objects.all()
...
def product_detail_view(request,pid):
    ...
    # getting all the reviews related to the products
...
def cart_view(request):
    ...
    if 'cart_data_obj' in request.session:
        for p_id, item in request.session['cart_data_obj'].items():
            ...
            # delivery_fee = (105/100) * float(cart_total_amount)
            # delivery_fee_total = (5/100) * float(cart_total_amount)
...
def best_price(request):
    return render(request, "core/best_price.html")
def wide_assortment(request):
    return render(request, "core/assortment.html")
def free_delivery(request):
    return render(request, "core/free_delivery.html")
def easy_returns(request):
    return render(request, "core/easy_returns.html")
def great_deal(request):
    return render(request, "core/great_deal.html")
def satisfaction(request):
    return render(request, "core/satisfaction.html")
...
def newsletter(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail = EmailMessage(subject, email_message, f"Explorer <{request.user.email}>", bcc=receivers,)
